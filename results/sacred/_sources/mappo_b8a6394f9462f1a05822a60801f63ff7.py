...
class MAPPO:
    ...
    def __init__(self, env, state_dim, action_dim,
                 ...
                 use_cuda=True, traffic_density=1, reward_type="global_R", epoch=10):
    ...
    def train(self):
        ...
        for agent_id in range(rewards_var.shape[0]):
            ...
            action_dists = th.distributions.Normal(mu, std)
            old_action_log_probs = action_dists.log_prob(actions_var[agent_id, :, :, :])
            for _ in range(self.epoch):
                mu, std = self.actor(states_var[agent_id, :, :, :])
                action_dists = th.distributions.Normal(mu, std)
                action_log_probs = action_dists.log_prob(actions_var[agent_id, :, :, :])
