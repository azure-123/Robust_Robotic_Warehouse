...
def tar_attack(agents, epsilon, states, action, tar_action):
    ...
    for i in range(len(agents)):
        ...
        action[i] = torch.stack(action[i])
        tar_action[i] = torch.stack(tar_action[i])
    for adv_state in adv_states:
        adv_state.retain_grad()
    losses = [-loss_func(torch.tensor(logits[i]), torch.tensor(tar_action[i])) + loss_func(torch.tensor(logits[i]), torch.tensor(action[i])) for i in range(len(agents))]
    for loss in losses:
        loss.backward()
    
    eta = [epsilon * adv_state[i].grad.data.sign() for i in range(len(agents))]
