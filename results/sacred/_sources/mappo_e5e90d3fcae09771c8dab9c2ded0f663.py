...
class MAPPO:
    ...
    def train(self):
        ...
        mu, std = self.actor(states_var)
        action_dists = th.distributions.Normal(mu, std)
        old_action_log_probs = action_dists.log_prob(actions_var)
        for _ in range(self.epoch):
            # update actor network
            self.actor_optimizer.zero_grad()
            mu, std = self.actor(states_var)
            action_dists = th.distributions.Normal(mu, std)
            action_log_probs = action_dists.log_prob(actions_var)
