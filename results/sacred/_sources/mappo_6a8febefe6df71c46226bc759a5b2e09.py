...
class MAPPO:
    ...
    def train(self):
        ...
        states_var = th.stack([self.storage[i].obs[:-1, :, :] for i in range(len(self.storage))])
        actions_var = th.stack([self.storage[i].actions for i in range(len(self.storage))])
        rewards_var = th.stack([self.storage[i].rewards for i in range(len(self.storage))])
        ...
        for agent_id in range(rewards_var.shape[0]):
            ...
            mu, std = self.actor(states_var[agent_id, :, :, :])
            action_log_probs = th.distributions.Normal(mu, std)
            # action_log_probs = th.sum(action_log_probs * actions_var[agent_id, :, :, :], 0)
            old_action_log_probs = action_log_probs.log_prob(actions_var[agent_id, :, :, :])
            # old_action_log_probs = self.actor_target(states_var[agent_id, :, :, :]).detach()
            old_action_log_probs = th.sum(old_action_log_probs * actions_var[agent_id, :, :, :], 0)
