...
def tar_attack(agents, epsilon, states, action, tar_action):
    ...
    adv_states = zip(*[torch.tensor(np.array(states[i]), requires_grad=True, dtype=torch.float32) for i in range(len(states))])
    logits = zip(*[agent.model(adv_states[agent.agent_id]) for agent in agents])
    # if use_cuda:
