        ...
        if adv == "adv_tar":
            ...
            for ep in range(self.arglist.num_episodes):
                ...
                while not done and episode_length < self.arglist.max_episode_len:
                    ...
                    self.adv_memory.add(obs, tar_onehot_actions, [-reward for reward in rewards], next_obs, dones)
        ...
        if adv == "atla":
            ...
            for ep in range(self.arglist.num_episodes):
                ...
                while not done and episode_length < self.arglist.max_episode_len:
                    ...
                    actions, onehot_actions = self.select_actions(torch_obs)
                    adv_obs = fgsm(self.alg, self.arglist.attack_epsilon, torch_obs, actions, self.alg.agents[0].optimizer)
                    # env_time += time.process_time() - timer
                    # timer = time.process_time()
                    actions, onehot_actions = self.select_actions(adv_obs)
                    # step_time += time.process_time() - timer
            ...
            for ep in range(self.arglist.num_episodes):
                ...
                while not done and episode_length < self.arglist.max_episode_len:
                    ...
                    adv_obs = rand_noise(self.arglist.attack_epsilon, torch_obs)
                    # env_time += time.process_time() - timer
                    # timer = time.process_time()
                    actions, onehot_actions = self.select_actions(adv_obs)
                    # step_time += time.process_time() - timer
            ...
            for ep in range(self.arglist.num_episodes):
                ...
                while not done and episode_length < self.arglist.max_episode_len:
                    ...
                    adv_obs = gaussian_noise(self.arglist.attack_epsilon, torch_obs)
                    # env_time += time.process_time() - timer
                    # timer = time.process_time()
                    actions, onehot_actions = self.select_actions(adv_obs)
                    # step_time += time.process_time() - timer
