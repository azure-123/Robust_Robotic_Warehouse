...
class MAPPO:
    ...
    def train(self):
        ...
        states_var = th.stack(batch.states).view(-1, self.n_agents, self.state_dim)
        actions_var = th.stack(batch.actions).view(-1, self.n_agents, self.action_dim)
        rewards_var = th.stack(batch.rewards).view(-1, self.n_agents, 1).to(th.float32)
        if self.use_cuda:
            states_var.cuda()
            actions_var.cuda()
            rewards_var.cuda()
